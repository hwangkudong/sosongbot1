...
exports.delete_sosongbot_data = function() {
  sosongbot.remove({}, function(err,removed){
    console.log('Data deletion was succeed.')
  });
}
			...
			if (err) {
				...
				for ( var i = 0 ; i < jsonData.length; i++ ) {
					...
					if ( jsonData[i].isnew === true ) {
						var sosongbotuhdData = new sosongbotuhd({keyword:jsonData[i].keyword, answer:jsonData[i].answer});

    					sosongbotuhdData.save(function(err){
    						if(err){
        						console.error(err);
            					return;
        					} else {
...
exports.create_sosongbot_data = function() {
  ...
  fs.exists(path + '/models/data/sosongbot_data.json', function(exists) {
		...
		jsonFile.readFile(path + '/models/data/sosongbot_data.json', function(err, jsonData) {
          ...
          if ( jsonData[i].isnew === true ) {
            var sosongbotdata = new sosongbot({menu_nm:jsonData[i].menu_nm, content:jsonData[i].content, button:jsonData[i].button });
            		...
            		return;
        				...
        				console.log('sosongbot data insert success');
    					}
    				});
					}
				}
